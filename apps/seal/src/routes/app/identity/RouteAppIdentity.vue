...
<template>
  <section class="space-y-8 md:space-y-10">
    <SectionIntro
      ...
      description="Generate, import, export, and clear the local signer used by Seal. These controls support setup and recovery, not the main signing workflow."
    />
        ...
        <Card v-if="showImportPanel" variant="panel" padding="md" class="space-y-5">
          ...
          <div class="flex flex-wrap gap-3">
            <Button variant="primary" :loading="isImporting" @click="onImport">
              {{ isImporting ? "Importing..." : "Import signer" }}
            </Button>
          </div>
              ...
              <FormField label="Export payload" description="Back this up securely before moving devices or clearing storage.">
                <template #default="{ id, describedBy }">
                  <Textarea
                    ...
                    readonly
                    class="proof-shell-copy"
                    placeholder="Create or import a signer to export it."
                  />
                </template>
              </FormField>
            </AccordionItem>
