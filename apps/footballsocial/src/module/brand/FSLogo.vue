...
const { isWhiteLabel, host, colorTheme } = useWhiteLabel();
</script>
<template>
  <div class="flex mx-2 group h-full" v-if="!isWhiteLabel">
    ...
    <div
      class="font-bold text-base-content text-[78px] md:text-[96px] lg:text-[136px] uppercase anton-regular [transform:translateX(-40px)] md:[transform:translateX(-42px)] lg:[transform:translateX(-24px)]"
    >
      Football
    </div>
  </div>
  <div class="flex mx-2 group h-full" v-else>
    ...
    >
      ...
      <img
        v-if="host === 'teamwork'"
        class="grayscale"
        :class="{ 'filter invert': colorTheme === 'dark' }"
        src="../../assets/teamwork-logo-1color-2023.png"
      />
    </div>
    <!-- <div
