...
- Keep layers strict: storage, ledger, replay, schema, workspace, services, surfaces, tasks.
- UI routes and components must not talk directly to Solid adapters when a workspace service can own that boundary.
- Tasks and future app-local projections emit Concord commands and consume replay output. They do not own truth or storage policy.
- Multi-ledger replay belongs in `replay` and `workspace`, never inside a specific app surface.
