...
import ConcordTodoWorkspace from "./ConcordTodoWorkspace.vue";
import { useConcordOsKernel } from "@/modules/concord-os";

const route = useRoute();
const router = useRouter();
const kernel = useConcordOsKernel();
const host = kernel.appHost;
const todoDraft = kernel.todo;
  ...
  <div class="h-full min-h-0 overflow-auto">
    <template v-if="host.status.value === 'loading'">
      <div class="flex h-full min-h-[22rem] items-center justify-center">
        <div class="space-y-3 text-center">
          ...
          <p class="m-0 text-[11px] text-[var(--ui-fg-muted)]">Hosted app</p>
      ...
      <div class="flex h-full min-h-[22rem] items-center justify-center p-4">
        <Card variant="subtle" padding="sm" class="w-full max-w-lg space-y-4">
          <div class="space-y-2">
            ...
            <p class="m-0 text-[11px] text-[var(--ui-fg-muted)]">{{ host.error.value }}</p>
          </div>
          <Button size="sm" variant="secondary" @click="host.backToLibrary(router)">Back to library</Button>
        </Card>
      </div>
    </template>
    ...
    <template v-else-if="host.activeAppId.value === 'todo' && host.activeTarget.value">
      <div class="flex h-full min-h-0 flex-col overflow-hidden">
        <div class="border-b border-[color-mix(in_srgb,var(--ui-border)_45%,transparent)] px-4 py-2.5">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="space-y-1">
              <p class="m-0 text-sm text-[var(--ui-fg)]">{{ host.activeTarget.value.title }} · {{ host.activeAppLabel.value }}</p>
              <p class="m-0 text-[11px] text-[var(--ui-fg-muted)]">{{ hostStatus.detail }}</p>
            </div>
            <div class="flex items-center gap-2">
              <Badge :tone="hostStatus.tone" variant="soft">{{ hostStatus.label }}</Badge>
              <Button size="xs" variant="plain-secondary" @click="host.backToLibrary(router)">Back</Button>
            </div>
          </div>
        </div>
