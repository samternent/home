...
watch(
  ...
  (next, previous) => {
    if (next > 0 && previous === 0) {
      pushConsoleEvent("Working copy staged", `${next} entity change${next === 1 ? "" : "s"} ready`);
      return;
    }
...
watch(
  ...
  (next, previous) => {
    if (next && !previous) {
      pushConsoleEvent("Commit started", todoWorkingCopy.commitMessage.value || "Writing staged changes");
      return;
    }
...
<template>
  ...
  >
    <template #header-actions>
      <ThemeModeToggle />
      <Button
        v-if="solid.isAuthenticated.value"
        size="xs"
        variant="secondary"
        @click="logout"
      >
    ...
    <div class="h-full overflow-auto p-4">
      <div class="flex h-full min-h-[18rem] rounded-[1.25rem] border border-[color-mix(in_srgb,var(--ui-border)_82%,transparent)] bg-[color-mix(in_srgb,var(--ui-bg)_94%,black)]">
    ...
    <template #right-side>
      <div class="space-y-4 p-4" v-if="showLibraryInspector || showHostedInspector">
        ...
        <Card v-if="inspectorMode === 'meta'" variant="subtle" padding="sm" class="space-y-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <Badge tone="neutral" variant="soft">
              {{ route.path.startsWith('/app/open') ? appHost.activeAppLabel.value || 'app' : library.selectedItem.value?.kind || 'system' }}
            </Badge>
          </div>
          ...
          <div class="space-y-1">
            ...
            <p
              v-if="route.path.startsWith('/app/library') && library.selectedItem.value?.modifiedLabel"
          ...
          <template v-if="route.path.startsWith('/app/library') && library.selectedItem.value?.kind === 'ledger'">
            <Separator />
            <div class="space-y-2">
              <p class="m-0 text-[11px] uppercase tracking-[0.24em] text-[var(--ui-fg-muted)]">
                Capabilities
        ...
        <Card v-else-if="inspectorMode === 'history'" variant="subtle" padding="sm" class="space-y-3">
          <p class="m-0 text-[11px] uppercase tracking-[0.24em] text-[var(--ui-fg-muted)]">
            Recent activity
          </p>
          <div class="space-y-2">
    ...
    <template #console>
      ...
      <div v-else class="space-y-3">
        <div class="space-y-1">
          <p v-for="line in systemSummaryLines" :key="line" class="m-0 text-[11px] text-[var(--ui-fg-muted)]">
            {{ line }}
          </p>
        </div>
        <Separator />
