...
function tonelliShanks(P) {
	...
	return function tonelliSlow(Fp2, n) {
		if (Fp2.is0(n)) return n;
		if (FpLegendre(Fp2, n) !== 1) throw new Error("Cannot find square root");
		let M = S;
		let c = Fp2.mul(Fp2.ONE, cc);
		let t = Fp2.pow(n, Q);
		let R = Fp2.pow(n, Q1div2);
		while (!Fp2.eql(t, Fp2.ONE)) {
			if (Fp2.is0(t)) return Fp2.ZERO;
			let i = 1;
			let t_tmp = Fp2.sqr(t);
...
function FpPow(Fp2, num, power) {
	...
	if (power === _0n$4) return Fp2.ONE;
	if (power === _1n$4) return num;
	let p = Fp2.ONE;
	let d = num;
	while (power > _0n$4) {
		if (power & _1n$4) p = Fp2.mul(p, d);
		d = Fp2.sqr(d);
		power >>= _1n$4;
	}
	return p;
...
function createCurveFields(type, CURVE, curveOpts = {}, FpFnLE) {
	...
	for (const p of params) if (!Fp2.isValid(CURVE[p])) throw new Error(`CURVE.${p} must be valid field element of CURVE.Fp`);
...
var _0n$2 = BigInt(0), _1n$2 = BigInt(1), _2n$2 = BigInt(2), _8n$1 = BigInt(8);
...
function edwards(params, extraOpts = {}) {
	...
	if (!isEdValidXY(Fp2, CURVE, CURVE.Gx, CURVE.Gy)) throw new Error("bad curve params: generator point");
	...
	const toAffineMemo = memoized((p, iz) => {
		const { X, Y, Z } = p;
		const is0 = p.is0();
		if (iz == null) iz = is0 ? _8n$1 : Fp2.inv(Z);
		const x = modP(X * iz);
		const y = modP(Y * iz);
	...
	const _Point = class {
		...
		static fromAffine(p) {
			if (p instanceof _Point) throw new Error("extended point not allowed");
			const { x, y } = p || {};
			acoord("x", x);
			acoord("y", y);
		...
		static fromBytes(bytes, zip215 = false) {
			...
			aInRange("point.y", y, _0n$2, zip215 ? MASK : Fp2.ORDER);
			const y2 = modP(y * y);
			let { isValid, value: x } = uvRatio2(modP(y2 - _1n$2), modP(d * y2 - a));
		...
		double() {
			...
			const Y3 = modP(G * H);
			const T3 = modP(E * H);
			return new _Point(X3, Y3, modP(F * G), T3);
		}
		add(other) {
			aedpoint(other);
			const { a, d } = CURVE;
		...
		multiply(scalar) {
			if (!Fn2.isValidNot0(scalar)) throw new Error("invalid scalar: expected 1 <= sc < curve.n");
		...
		multiplyUnsafe(scalar, acc = _Point.ZERO) {
			if (!Fn2.isValid(scalar)) throw new Error("invalid scalar: expected 0 <= sc < curve.n");
			if (scalar === _0n$2) return _Point.ZERO;
	...
	let Point = _Point;
	__publicField(Point, "BASE", new _Point(CURVE.Gx, CURVE.Gy, _1n$2, modP(CURVE.Gx * CURVE.Gy)));
	__publicField(Point, "ZERO", new _Point(_0n$2, _1n$2, _1n$2, _0n$2));
	__publicField(Point, "Fp", Fp2);
	__publicField(Point, "Fn", Fn2);
...
function eddsa(Point, cHash, eddsaOpts = {}) {
	...
	function hashDomainToScalar(context = Uint8Array.of(), ...msgs) {
		return modN_LE(cHash(domain(concatBytes$1(...msgs), abytes$1(context, void 0, "context"), !!prehash)));
	}
	function sign(msg, secretKey, options = {}) {
		...
		if (!Fn2.isValid(s)) throw new Error("sign failed: invalid s");
		return abytes$1(concatBytes$1(R, Fn2.toBytes(s)), lengths.signature, "result");
	}
	...
	function isValidSecretKey(key) {
		return isBytes$1(key) && key.length === Fn2.BYTES;
	}
	...
	const utils2 = {
		...
		toMontgomerySecret(secretKey) {
			...
			return adjustScalarBytes2(cHash(secretKey.subarray(0, size))).subarray(0, size);
		}
	};
...
var _0n = /* @__PURE__ */ BigInt(0), _1n = BigInt(1), _2n = BigInt(2);
...
var __RistrettoPoint = class extends PrimeEdwardsPoint {
	constructor(ep) {
		super(ep);
	}
	...
	toBytes() {
		...
		const u2 = mod2(X * Y);
		const { value: invsqrt } = invertSqrt(mod2(u1 * mod2(u2 * u2)));
...
__publicField(_RistrettoPoint, "BASE", /* @__PURE__ */ (() => new __RistrettoPoint(ed25519_Point.BASE))());
__publicField(_RistrettoPoint, "ZERO", /* @__PURE__ */ (() => new __RistrettoPoint(ed25519_Point.ZERO))());
__publicField(_RistrettoPoint, "Fp", /* @__PURE__ */ (() => Fp)());
__publicField(_RistrettoPoint, "Fn", /* @__PURE__ */ (() => Fn)());
var _HMAC = class {
...
function generateMnemonic$1(wordlist2, strength = 128) {
	anumber$1(strength);
...
function entropyToMnemonic(entropy, wordlist2) {
	aentropy(entropy);
	return getCoder(wordlist2).encode(entropy).join(isJapanese(wordlist2) ? "　" : " ");
}
function validateMnemonic$1(mnemonic, wordlist2) {
	try {
...
zone
zoo`.split("\n");
var IDENTITY_FORMAT = "ternent-identity";
var IDENTITY_VERSION = "2";
var IDENTITY_ALGORITHM = "Ed25519";
var ED25519_CONTEXT = "ternent-seal/v2";
var IDENTITY_MATERIAL_KIND = "seed";
var BIP39_LANGUAGE = "english";
var SLIP10_HARDENED_OFFSET = 2147483648;
var SLIP10_KEY = new TextEncoder().encode("ed25519 seed");
var IDENTITY_PATH_SEGMENTS = [
	101010,
	25519,
	0
];
var BECH32_GENERATORS = [
	996825010,
	642813549,
	513874426,
	1027748829,
...
function base64UrlDecode(value) {
	...
	if (!normalized) throw new Error("Base64url value is required.");
	return base64Decode(`${normalized}${normalized.length % 4 === 0 ? "" : "=".repeat(4 - normalized.length % 4)}`);
}
...
function assertWordCount(words) {
	if (words === 12 || words === 24) return words;
	throw new Error("Mnemonic word count must be 12 or 24.");
}
function strengthFromWordCount(words) {
...
function deriveSlip10Seed(seedBytes) {
	let chain2 = hmac(sha512, SLIP10_KEY, seedBytes);
	let key = chain2.slice(0, 32);
	let chainCode = chain2.slice(32);
	for (const segment of IDENTITY_PATH_SEGMENTS) {
		const index = segment + SLIP10_HARDENED_OFFSET;
		chain2 = hmac(sha512, chainCode, concatBytes(new Uint8Array([0]), key, serializeUint32(index)));
		key = chain2.slice(0, 32);
		chainCode = chain2.slice(32);
	}
	return key;
}
...
function resolveSeedBytes(input) {
	...
	if (isSerializedIdentity(input)) return resolveSeedBytes(input.material.seed);
...
function resolvePublicKeyBytes(input) {
	...
	if (isSerializedIdentity(input)) return resolvePublicKeyBytes(input.publicKey);
	if (isRecord(input) && "publicKey" in input) return resolvePublicKeyBytes(input.publicKey);
	throw new Error("A valid 32-byte public key is required.");
}
function resolveX25519PrivateKeyBytes(input) {
...
function bech32Polymod(values) {
	...
	for (const value of values) {
		...
		checksum2 = (checksum2 & 33554431) << 5 ^ value;
		for (let bit = 0; bit < 5; bit += 1) if (top >>> bit & 1) checksum2 ^= BECH32_GENERATORS[bit];
	}
	return checksum2;
}
...
function convertBits(data, fromBits, toBits) {
	...
	if (bits > 0) output.push(value << toBits - bits & maxValue);
	return output;
}
function bech32CreateChecksum(hrp, data) {
	...
	const checksum2 = [];
	for (let index = 0; index < 6; index += 1) checksum2.push(polymod >> 5 * (5 - index) & 31);
	return checksum2;
}
function bech32Encode(hrp, data) {
	const charset = "qpzry9x8gf2tvdw0s3jn54khce6mua7l";
	const words = convertBits(data, 8, 5);
	return `${hrp}1${[...words, ...bech32CreateChecksum(hrp, words)].map((value) => charset[value]).join("")}`;
}
async function createIdentityFromSeedBytes(seedBytes, createdAt) {
...
function generateMnemonic(input = {}) {
	return generateMnemonic$1(wordlist, strengthFromWordCount(assertWordCount(input.words ?? 12)));
}
function validateMnemonic(mnemonic, options = {}) {
	if (options.wordlist && options.wordlist !== BIP39_LANGUAGE) throw new Error("Only the English BIP-39 wordlist is supported.");
	return validateMnemonic$1(String(mnemonic || "").trim(), wordlist);
}
async function createIdentityFromMnemonic(input) {
	const mnemonic = String(input.mnemonic || "").trim();
	if (!validateMnemonic(mnemonic)) throw new Error("Mnemonic phrase is not a valid English BIP-39 phrase.");
	return createIdentityFromSeedBytes(deriveSlip10Seed(mnemonicToSeedSync(mnemonic, input.passphrase || "")), input.createdAt ?? (/* @__PURE__ */ new Date()).toISOString());
}
async function createMnemonicIdentity(input = {}) {
...
function parseIdentity(input) {
	...
	if (!isSerializedIdentity(parsed)) throw new Error("Identity payload must be a ternent-identity v2 object.");
	return toCanonicalIdentity(parsed);
}
async function validateIdentity$1(identity) {
	const parsed = parseIdentity(identity);
	if (await derivePublicKey(parsed.material.seed) !== normalizeBase64Url(parsed.publicKey)) throw new Error("Identity publicKey does not match the stored seed.");
	if (await deriveKeyId(parsed.publicKey) !== parsed.keyId) throw new Error("Identity keyId does not match the public key.");
	return parsed;
}
async function signBytes(identityOrSeed, payload, options = {}) {
	return base64UrlEncode(ed25519.sign(combineContext(ensureBytes(payload, "Payload"), options.context), resolveSeedBytes(identityOrSeed)));
}
async function verifyBytes(publicKey, payload, signature, options = {}) {
	return ed25519.verify(base64UrlDecode(signature), combineContext(ensureBytes(payload, "Payload"), options.context), resolvePublicKeyBytes(publicKey));
}
...
var RageInitError = class extends RageError {};
var RageValidationError = class extends RageError {};
var RageEncryptionError = class extends RageError {};
var RageDecryptionError = class extends RageError {};
...
function deserializeError(error) {
	if (error.code) {
		const hydrated$1 = new (getRageErrorConstructor(error.name, error.code))(error.code, error.message, error.cause);
		if (error.stack) hydrated$1.stack = error.stack;
		return hydrated$1;
	}
	const hydrated = new Error(error.message);
	hydrated.name = error.name;
	if (error.stack) hydrated.stack = error.stack;
	if (error.cause !== void 0) Reflect.set(hydrated, "cause", error.cause);
	return hydrated;
}
function getRageErrorConstructor(name, code) {
	switch (name) {
		case "RageValidationError": return RageValidationError;
		case "RageEncryptionError": return RageEncryptionError;
		case "RageDecryptionError": return RageDecryptionError;
		case "RageInitError": return RageInitError;
	}
	switch (code) {
		...
		case "RAGE_DATA_TOO_LARGE": return RageValidationError;
		case "RAGE_ENCRYPT_FAILED": return RageEncryptionError;
		case "RAGE_DECRYPT_FAILED": return RageDecryptionError;
		case "RAGE_INIT_FAILED":
		default: return RageInitError;
	}
}
function toEncryptionError(error) {
	if (error instanceof RageError) return error;
	const message = getMessage(error).toLowerCase();
	if (message.includes("invalid recipient")) return new RageValidationError("RAGE_INVALID_RECIPIENT", "Recipient must be an age recipient string.", error);
	if (message.includes("data too large")) return new RageValidationError("RAGE_DATA_TOO_LARGE", "Data exceeds the 64MB maximum message size.", error);
	return new RageEncryptionError("RAGE_ENCRYPT_FAILED", "Failed to encrypt data.", error);
}
function toDecryptionError(error) {
	if (error instanceof RageError) return error;
	if (getMessage(error).toLowerCase().includes("invalid identity")) return new RageValidationError("RAGE_INVALID_IDENTITY", "Identity must be an age secret key string.", error);
...
function __wbg_adapter_24(arg0, arg1, arg2) {
	wasm.wasm_bindgen__convert__closures__invoke1_mut__hace5475a0d990476(arg0, arg1, addHeapObject(arg2));
}
function generate_keypair() {
	return takeObject(wasm.generate_keypair());
}
...
function encrypt_with_recipients(recipients, data, armor) {
	const ptr0 = passArray8ToWasm0(data, wasm.__wbindgen_malloc);
	const len0 = WASM_VECTOR_LEN;
	return takeObject(wasm.encrypt_with_recipients(addHeapObject(recipients), ptr0, len0, armor));
}
function decrypt_with_identity(identity, data) {
	const ptr0 = passStringToWasm0(identity, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
	const len0 = WASM_VECTOR_LEN;
	const ptr1 = passArray8ToWasm0(data, wasm.__wbindgen_malloc);
	const len1 = WASM_VECTOR_LEN;
	return takeObject(wasm.decrypt_with_identity(ptr0, len0, ptr1, len1));
}
function encrypt_with_passphrase(passphrase, data, armor) {
	const ptr0 = passStringToWasm0(passphrase, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
	const len0 = WASM_VECTOR_LEN;
	const ptr1 = passArray8ToWasm0(data, wasm.__wbindgen_malloc);
	const len1 = WASM_VECTOR_LEN;
	return takeObject(wasm.encrypt_with_passphrase(ptr0, len0, ptr1, len1, armor));
}
function decrypt_with_passphrase(passphrase, data) {
	const ptr0 = passStringToWasm0(passphrase, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
	const len0 = WASM_VECTOR_LEN;
	const ptr1 = passArray8ToWasm0(data, wasm.__wbindgen_malloc);
	const len1 = WASM_VECTOR_LEN;
	return takeObject(wasm.decrypt_with_passphrase(ptr0, len0, ptr1, len1));
}
...
function __wbg_adapter_64(arg0, arg1, arg2, arg3) {
	wasm.wasm_bindgen__convert__closures__invoke2_mut__h532aaa040d7fdd0a(arg0, arg1, addHeapObject(arg2), addHeapObject(arg3));
}
...
function __wbg_get_imports() {
	...
	imports.wbg.__wbg_new_81740750da40724f = function(arg0, arg1) {
		try {
			...
			var cb0 = (arg0$1, arg1$1) => {
				...
				state0.a = 0;
				try {
					return __wbg_adapter_64(a, state0.b, arg0$1, arg1$1);
				} finally {
					state0.a = a;
				}
			};
	...
	imports.wbg.__wbindgen_closure_wrapper1159 = function(arg0, arg1, arg2) {
		return addHeapObject(makeMutClosure(arg0, arg1, 40, __wbg_adapter_24));
	};
	return imports;
}
...
async function __wbg_init(input) {
	...
	if (typeof input === "undefined") input = new URL("ternent_rage_wasm_bg.wasm", import.meta.url);
...
async function loadWasmBindings() {
	if (bindings) return;
	try {
		await ternent_rage_wasm_default(decodeBase64(wasmBase64));
		bindings = {
			generateKeyPair: generate_keypair,
			encryptWithRecipients: encrypt_with_recipients,
			decryptWithIdentity: decrypt_with_identity,
			encryptWithPassphrase: encrypt_with_passphrase,
			decryptWithPassphrase: decrypt_with_passphrase
		};
	} catch (error) {
		bindings = null;
...
function getWasmBindings() {
	if (!bindings) throw new RageInitError("RAGE_INIT_FAILED", "Rage WASM is not initialized. Call initRage() before using @ternent/rage.");
	return bindings;
}
function createDirectRageRuntime() {
	return {
		init: loadWasmBindings,
		generateKeyPair: () => getWasmBindings().generateKeyPair(),
		encryptWithRecipients: (recipients, data, armor) => getWasmBindings().encryptWithRecipients(recipients, data, armor),
		decryptWithIdentity: (identity, data) => getWasmBindings().decryptWithIdentity(identity, data),
		encryptWithPassphrase: (passphrase, data, armor) => getWasmBindings().encryptWithPassphrase(passphrase, data, armor),
		decryptWithPassphrase: (passphrase, data) => getWasmBindings().decryptWithPassphrase(passphrase, data)
	};
}
function WorkerWrapper(options) {
	return new Worker("/assets/worker-zDKjw8fQ.js", { name: options?.name });
}
function getTransferList(message) {
	if ("data" in message && message.data instanceof Uint8Array) return [message.data.buffer];
	if ("value" in message && message.value instanceof Uint8Array) return [message.value.buffer];
	return [];
}
function cloneBytes(data) {
	return Uint8Array.from(data);
}
var WorkerRageRuntime = class {
	worker = null;
	nextRequestId = 0;
	ready = false;
	initPromise = null;
	...
	async init() {
		...
		if (!this.initPromise) this.initPromise = this.request({ type: "init" }).then(() => {
			this.ready = true;
		}).catch((error) => {
			this.resetWorker();
			throw error;
	...
	async generateKeyPair() {
		...
		return this.request({ type: "generateKeyPair" });
	}
	async encryptWithRecipients(recipients, data, armor) {
		await this.ensureReady();
		return this.request({
	...
	async decryptWithIdentity(identity, data) {
		await this.ensureReady();
		return this.request({
	...
	async encryptWithPassphrase(passphrase, data, armor) {
		await this.ensureReady();
		return this.request({
	...
	async decryptWithPassphrase(passphrase, data) {
		await this.ensureReady();
		return this.request({
	...
	request(request) {
		...
		return new Promise((resolve, reject) => {
			...
			try {
				this.getWorker().postMessage(message, getTransferList(message));
			} catch (error) {
	...
	handleError = (event) => {
		const message = typeof event.message === "string" && event.message.length > 0 ? event.message : "Rage worker failed.";
		this.handleFatalError(new RageInitError("RAGE_INIT_FAILED", message, event));
	};
	handleMessageError = () => {
		this.handleFatalError(new RageInitError("RAGE_INIT_FAILED", "Rage worker failed to exchange messages."));
	};
...
async function loadRageRuntime() {
	...
	try {
		...
		if (runtimeMode !== "worker") {
			runtime = null;
			runtimeMode = null;
			if (error instanceof RageInitError) throw error;
			throw new RageInitError("RAGE_INIT_FAILED", "Failed to initialize Rage WASM.", error);
		}
		runtimeMode = "direct";
		runtime = createRuntime(runtimeMode);
		try {
			...
			runtime = null;
			runtimeMode = null;
			if (fallbackError instanceof RageInitError) throw fallbackError;
			throw new RageInitError("RAGE_INIT_FAILED", "Failed to initialize Rage WASM.", fallbackError);
		}
	}
}
function getRageRuntime() {
	if (!runtime) throw new RageInitError("RAGE_INIT_FAILED", "Rage WASM is not initialized. Call initRage() before using @ternent/rage.");
	return runtime;
}
var initialized = false;
var initPromise = null;
async function initRage() {
	if (initialized) return;
	if (!initPromise) initPromise = loadRageRuntime();
	try {
		await initPromise;
		initialized = true;
	} catch (error) {
...
function assertRageInitialized() {
	if (!initialized) throw new RageInitError("RAGE_INIT_FAILED", "Rage WASM is not initialized. Call initRage() before using @ternent/rage.");
}
new TextEncoder();
new TextDecoder("utf-8", { fatal: true });
function validatePassphrase(passphrase) {
	if (typeof passphrase !== "string" || passphrase.length === 0) throw new RageValidationError("RAGE_EMPTY_PASSPHRASE", "Passphrase must not be empty.");
}
...
function validateData$1(data) {
	const normalized = normalizeData$1(data);
	if (normalized.byteLength === 0) throw new RageValidationError("RAGE_EMPTY_DATA", "Data must be a non-empty Uint8Array.");
	if (normalized.byteLength > 67108864) throw new RageValidationError("RAGE_DATA_TOO_LARGE", "Data exceeds the 64MB maximum message size.");
	return normalized;
}
function normalizeCiphertext$2(value, output) {
	if (!(value instanceof Uint8Array)) throw new RageEncryptionError("RAGE_ENCRYPT_FAILED", `Rage WASM returned an invalid ${output} ciphertext payload.`);
	return value;
}
function normalizePlaintext$1(value) {
	if (!(value instanceof Uint8Array)) throw new RageDecryptionError("RAGE_DECRYPT_FAILED", "Rage WASM returned an invalid plaintext payload.");
	return value;
}
async function encryptWithPassphrase$1(input) {
	assertRageInitialized();
	validatePassphrase(input.passphrase);
	const data = validateData$1(input.data);
	const output = input.output ?? "armor";
	try {
		return normalizeCiphertext$2(await getRageRuntime().encryptWithPassphrase(input.passphrase, data, output === "armor"), output);
	} catch (error) {
		throw toEncryptionError(error);
	}
}
async function decryptWithPassphrase$1(input) {
	assertRageInitialized();
	validatePassphrase(input.passphrase);
	const data = validateData$1(input.data);
	try {
		return normalizePlaintext$1(await getRageRuntime().decryptWithPassphrase(input.passphrase, data));
	} catch (error) {
...
function validateRecipients(recipients) {
	if (!Array.isArray(recipients) || recipients.length === 0) throw new RageValidationError("RAGE_EMPTY_RECIPIENTS", "At least one recipient is required.");
	for (const recipient of recipients) if (!String(recipient).trim().startsWith("age1")) throw new RageValidationError("RAGE_INVALID_RECIPIENT", "Recipient must be an age recipient string.");
}
function validateIdentity(identity) {
	const normalized = String(identity).trim();
	if (!normalized.startsWith("AGE-SECRET-KEY-")) throw new RageValidationError("RAGE_INVALID_IDENTITY", "Identity must be an age secret key string.");
	return normalized;
}
function normalizeCiphertext$1(value, output) {
	if (!(value instanceof Uint8Array)) throw new RageEncryptionError("RAGE_ENCRYPT_FAILED", `Rage WASM returned an invalid ${output} ciphertext payload.`);
	return value;
}
...
async function encryptWithRecipients(input) {
	assertRageInitialized();
	validateRecipients(input.recipients);
	const data = validateData(input.data);
	const output = input.output ?? "armor";
	try {
		return normalizeCiphertext$1(await getRageRuntime().encryptWithRecipients(input.recipients, data, output === "armor"), output);
	} catch (error) {
		throw toEncryptionError(error);
	}
}
async function decryptWithIdentity$1(input) {
	assertRageInitialized();
	const identity = validateIdentity(input.identity);
	const data = validateData(input.data);
	try {
		return normalizePlaintext(await getRageRuntime().decryptWithIdentity(identity, data));
	} catch (error) {
		throw toDecryptionError(error);
	}
}
var ArmourError = class extends Error {
	code;
	cause;
	constructor(code, message, cause) {
		super(message);
...
var ArmourInitError = class extends ArmourError {};
var ArmourValidationError = class extends ArmourError {};
var ArmourIdentityError = class extends ArmourError {};
var ArmourEncryptionError = class extends ArmourError {};
var ArmourDecryptionError = class extends ArmourError {};
function toArmourInitError(error) {
	if (error instanceof ArmourInitError) return error;
	return new ArmourInitError("ARMOUR_INIT_FAILED", "Failed to initialize Armour.", error);
}
function toArmourIdentityError(error, code, fallbackMessage) {
	if (error instanceof ArmourIdentityError) return error;
	return new ArmourIdentityError(code, fallbackMessage, error);
}
function toArmourEncryptionError(error) {
	if (error instanceof ArmourError) return error;
	if (error instanceof RageInitError) return new ArmourInitError("ARMOUR_INIT_FAILED", "Armour is not initialized. Call initArmour() before encrypting or decrypting.", error);
	if (error instanceof RageValidationError) switch (error.code) {
		case "RAGE_EMPTY_DATA": return new ArmourValidationError("ARMOUR_EMPTY_DATA", "Data must be a non-empty Uint8Array.", error);
		case "RAGE_EMPTY_RECIPIENTS": return new ArmourValidationError("ARMOUR_EMPTY_RECIPIENTS", "At least one recipient is required.", error);
		case "RAGE_INVALID_RECIPIENT": return new ArmourValidationError("ARMOUR_INVALID_RECIPIENT", "Recipient must be an age recipient string.", error);
		case "RAGE_EMPTY_PASSPHRASE": return new ArmourValidationError("ARMOUR_EMPTY_PASSPHRASE", "Passphrase must not be empty.", error);
		case "RAGE_DATA_TOO_LARGE": return new ArmourValidationError("ARMOUR_DATA_TOO_LARGE", "Data exceeds the 64MB maximum message size.", error);
		default: break;
	}
	if (error instanceof RageEncryptionError || error instanceof RageError) return new ArmourEncryptionError("ARMOUR_ENCRYPT_FAILED", "Failed to encrypt data.", error);
	return new ArmourEncryptionError("ARMOUR_ENCRYPT_FAILED", "Failed to encrypt data.", error);
}
function toArmourDecryptionError(error) {
	if (error instanceof ArmourError) return error;
	if (error instanceof RageInitError) return new ArmourInitError("ARMOUR_INIT_FAILED", "Armour is not initialized. Call initArmour() before encrypting or decrypting.", error);
	if (error instanceof RageValidationError) switch (error.code) {
		case "RAGE_EMPTY_DATA": return new ArmourValidationError("ARMOUR_EMPTY_DATA", "Data must be a non-empty Uint8Array.", error);
		case "RAGE_INVALID_IDENTITY": return new ArmourValidationError("ARMOUR_INVALID_SECRET_KEY", "Secret key must be an age secret key string.", error);
		case "RAGE_EMPTY_PASSPHRASE": return new ArmourValidationError("ARMOUR_EMPTY_PASSPHRASE", "Passphrase must not be empty.", error);
		default: break;
	}
	if (error instanceof RageDecryptionError || error instanceof RageError) return new ArmourDecryptionError("ARMOUR_DECRYPT_FAILED", "Failed to decrypt data.", error);
	return new ArmourDecryptionError("ARMOUR_DECRYPT_FAILED", "Failed to decrypt data.", error);
}
function invalidUtf8Error(error) {
	return new ArmourDecryptionError("ARMOUR_DECRYPT_FAILED", "Decrypted data is not valid UTF-8.", error);
}
...
async function resolveIdentity(input) {
	try {
		...
		throw toArmourIdentityError(error, "ARMOUR_INVALID_IDENTITY", "Identity input must be a valid @ternent/identity serialized identity.");
	}
}
async function secretKeyFromIdentity(input) {
	const identity = await resolveIdentity(input);
	try {
		return await deriveAgeSecretKey(identity);
	} catch (error) {
		throw toArmourIdentityError(error, "ARMOUR_IDENTITY_DERIVATION_FAILED", "Failed to derive an age secret key from the provided identity.");
	}
}
async function encryptForRecipients(input) {
	try {
		return await encryptWithRecipients({
...
async function decryptWithIdentity(input) {
	try {
		return await decryptWithIdentity$1({
...
async function encryptWithPassphrase(input) {
	try {
		return await encryptWithPassphrase$1({
			passphrase: input.passphrase,
			data: input.data,
			output: input.output
		});
...
async function decryptWithPassphrase(input) {
	try {
		return await decryptWithPassphrase$1({
			passphrase: input.passphrase,
			data: input.data
		});
	} catch (error) {
...
function normalizeCiphertext(data) {
	return typeof data === "string" ? utf8Encoder.encode(data) : data;
}
async function encryptTextWithPassphrase(input) {
...
async function decryptTextWithPassphrase(input) {
	return decodeUtf8(await decryptWithPassphrase({
		passphrase: input.passphrase,
		data: normalizeCiphertext(input.data)
	}));
}
async function initArmour() {
	try {
		await initRage();
	} catch (error) {
...
export { verifyUtf8 as S, createMnemonicIdentity as _, encryptWithPassphrase as a, signUtf8 as b, resolveIdentity as c, ArmourEncryptionError as d, ArmourError as f, createIdentityFromMnemonic as g, ArmourValidationError as h, decryptWithPassphrase as i, secretKeyFromIdentity as l, ArmourInitError as m, decryptTextWithPassphrase as n, decryptWithIdentity as o, ArmourIdentityError as p, encryptTextWithPassphrase as r, encryptForRecipients as s, initArmour as t, ArmourDecryptionError as u, deriveKeyId as v, validateIdentity$1 as x, parseIdentity as y };
