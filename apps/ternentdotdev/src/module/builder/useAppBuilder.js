...
export function provideAppBuilder() {
  ...
  async function removeSchema(appId, schemaId) {
    const schemaCollection = getCollection("app_schemas");
    const schemaItem = schemaCollection?.data?.find(item => 
      item.data.id === schemaId && item.data.appId === appId
    );
  ...
  async function removeView(appId, viewId) {
    const viewCollection = getCollection("app_views");
    const viewItem = viewCollection?.data?.find(item => 
      item.data.id === viewId && item.data.appId === appId
    );
  ...
  async function migrateTasks() {
    ...
    const taskSchema = await createSchema(tasksAppId, {
      ...
      fields: [
        ...
        { id: generateId(), name: "completed", type: "boolean", required: false },
        { id: generateId(), name: "description", type: "textarea", required: false }
      ]
    });
