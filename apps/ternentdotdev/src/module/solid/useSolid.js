  ...
  async function saveLedgerToSolid(ledgerData, filename = "ledger.json") {
    ...
    try {
      ...
      if (profile.value?.storageAll && profile.value.storageAll.length > 0) {
        ...
        profile.value?.webIdProfile?.storage &&
        profile.value.webIdProfile.storage.length > 0
      ) {
        ...
        const storagePredicates =
          webIdSubject?.predicates?.["http://www.w3.org/ns/pim/space#storage"];
      ...
      try {
        ...
        if (containerUrl !== storageUrl && !containerUrl.endsWith("public/")) {
          try {
            ...
            console.log(
              "⚠️ Container creation failed (might already exist):",
    ...
    throw new Error(
      `Failed to save ledger to your Solid pod storage. Your provider might not allow file creation in the attempted locations.`,
    );
  }
  ...
  async function listLedgersFromSolid() {
    ...
    for (let i = 0; i < containerUrls.length; i++) {
      ...
      try {
        ...
        console.log(
          `Found ${containedResourceUrls.length} resources in ${containerUrl}:`,
        ...
        for (const resourceUrl of containedResourceUrls) {
          ...
          if (
            (filename.endsWith(".json") || filename.endsWith(".ledger.json")) &&
        ...
        try {
          ...
          if (profileThing) {
            ...
            try {
              ...
              console.log(
                "getProfileAll failed, using basic profile data:",
                profileAllError.message,
              );
          ...
          console.log(
            "Profile fetch failed for existing session, but session is valid:",
        ...
        try {
          ...
          if (profileThing) {
            try {
              ...
              console.log(
                "getProfileAll failed after restore, using basic profile data:",
                profileAllError.message,
              );
              profile.value = {
                webId: webId.value,
          ...
          console.log("Solid session restored successfully:", {
            webId: webId.value,
            profile: profile.value,
          });
        } catch (profileError) {
          console.log(
            "Solid profile fetch had issues, but session is valid. This is often normal:",
            profileError.message,
          );
