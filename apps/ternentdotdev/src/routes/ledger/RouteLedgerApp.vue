...
const schemas = computed(() => getAppSchemas(appId.value));

useBreadcrumbs({
  path: computed(() => `/ledger/app/${appId.value}`),
  name: computed(() => app.value ? `${app.value.name}` : "App"),
});
...
watch([schemas, () => appId.value], ([newSchemas]) => {
...
function updateCurrentSchemaData() {
  ...
  const data = getSchemaData(appId.value, activeSchema.value.id);
  ...
  <div class="flex-1 p-6 max-w-6xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
      ...
      <div class="flex gap-2">
        <SButton :to="`/builder/app/${appId}/design`" class="btn-outline">
    ...
    <div v-else-if="schemas.length === 0" class="text-center py-12">
      ...
      <SButton :to="`/builder/app/${appId}/design`" class="btn-primary">
        Add Data Types
      </SButton>
    </div>
