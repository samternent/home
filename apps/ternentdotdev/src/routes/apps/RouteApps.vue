...
function getLinkedSchemaData(schemaId) {
  ...
  console.log('getLinkedSchemaData result:', { schemaId, data, length: data?.length });
...
onMounted(() => {
  ...
  if ((route.path === '/apps' || route.path === '/apps/') && safeApps.value.length > 0) {
    router.replace(`/apps/${safeApps.value[0].id}`);
  }
});
</script>
        ...
        <div v-if="hasApps && currentApp" class="flex-1 p-6 max-w-6xl mx-auto">
          <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex gap-2">
              <SButton
                :to="`/builder/app/${currentAppId}/design`"
                class="btn-outline"
              >
                        ...
                        <span v-else-if="field.type === 'link'">
                          {{
                            (() => {
                              ...
                              const linkedSchema = schemas.find(s => s.id === field.linkedSchema);
                              const linkedData = getLinkedSchemaData(field.linkedSchema);
                              const linkedItem = linkedData.find(d => d.id === item[field.name]);
                              return linkedItem ? getLinkedDataDisplayValue(linkedItem, linkedSchema) : `Missing (${item[field.name]})`;
                            })()
                          }}
                        </span>
                        <span v-else>
        ...
        >
          <div class="p-4 space-y-4">
            <div class="alert alert-info">
              ...
              <div class="text-sm">
                Adding a new {{ activeSchema?.name.slice(0, -1).toLowerCase() || 'item' }} to your {{ currentApp?.name || 'app' }}.
              </div>
            </div>
              ...
              <div v-else-if="field.type === 'link'">
                ...
                >
                  ...
                  <option 
                    ...
                    :value="item.id"
                  >
                    {{ getLinkedDataDisplayValue(item, schemas.find(s => s.id === field.linkedSchema)) }}
                  </option>
                </select>
                <div class="label">
                  <span class="label-text-alt text-base-content/60">
                    Links to {{ schemas.find(s => s.id === field.linkedSchema)?.name || 'another data type' }}
                  </span>
                </div>
              </div>
            ...
            <div class="flex gap-2 pt-4">
              ...
              <SButton @click="cancelForm" type="ghost">
                Cancel
              </SButton>
            </div>
          </div>
