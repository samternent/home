...
function isActiveUser(identity: string) {
  return people.value?.includes((person: IRecord): Boolean => {
    console.log(person);
    return person.data?.identity === identity;
  });
}
</script>
<template>
  <div class="w-full flex-1 mx-auto max-w-6xl px-8 flex flex-col animate">
    ...
    <div class="mt-4">
      ...
      >
        ...
        <button
          v-if="!isActiveUser(user.public.identity)"
          @click="addPerson(user.public)"
          class="px-4 py-2 text-lg bg-green-600 hover:bg-green-700 transition-all rounded-full flex items-center font-medium"
        >
          <svg
