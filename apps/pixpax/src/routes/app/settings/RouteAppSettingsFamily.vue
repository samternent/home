...
const activeChildLabel = computed(
  () =>
    family.activeManagedUser.value?.displayName ||
    identities.identity.value?.displayName ||
    "Current child",
);
</script>
...
<template>
  <div class="space-y-6">
    <Card variant="showcase" padding="sm" class="space-y-5">
      <div class="space-y-1">
        ...
        <p class="m-0 text-sm text-[var(--ui-fg-muted)]">
          Adults sign in. Children do not. Save child identities and Pixbooks to the family space,
          then open them on another trusted device when needed.
        </p>
      </div>
      ...
      <div v-if="family.account.isAuthenticated.value" class="space-y-3">
        <p class="m-0 text-sm text-[var(--ui-fg-muted)]">
          ...
          <span class="text-[var(--ui-fg)]">{{
            family.account.user.value?.email ||
            family.account.user.value?.name ||
            family.account.user.value?.id
          }}</span>
        </p>
      ...
      <div v-else class="grid gap-3">
        <p class="m-0 text-sm text-[var(--ui-fg-muted)]">
          Adults sign in here to unlock family backup. Children stay local and never need their own
          account.
        </p>
        <div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
          ...
          <Button
            size="sm"
            variant="secondary"
            :disabled="family.authBusy.value"
            @click="family.sendOtpCode()"
          >
            Email code
          </Button>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
          <Button
            ...
            variant="plain-secondary"
            :disabled="family.authBusy.value"
            @click="family.signInWithPasskey()"
          >
            Use passkey
          </Button>
        </div>
      ...
      <p v-if="family.authMessage.value" class="m-0 text-sm text-[var(--ui-fg-muted)]">
        {{ family.authMessage.value }}
      </p>
    </Card>
    ...
    <Card variant="subtle" padding="sm" class="space-y-4">
      ...
      <div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
        ...
        <Button
          size="sm"
          variant="secondary"
          @click="family.unlockRecoveryPassphrase(family.recoveryPassphrase.value)"
        >
          Unlock
        </Button>
      ...
      <p
        class="m-0 text-sm"
        :class="
          family.recoveryPassphraseUnlocked.value
            ? 'text-[var(--ui-accent)]'
      ...
      >
        {{
          family.recoveryPassphraseUnlocked.value
            ...
            : "Passphrase locked. Child key backup is paused."
        }}
      </p>
    </Card>
    ...
    <Card variant="subtle" padding="sm" class="space-y-4">
      ...
      <div
        class="rounded-3xl border border-[var(--ui-border)] bg-[rgba(255,255,255,0.03)] p-4 text-sm text-[var(--ui-fg-muted)]"
      >
        <p class="m-0">
          Last family save:
          <span class="text-[var(--ui-fg)]">{{
            family.snapshotState.value.savedAt || "Not saved yet"
          }}</span>
        </p>
        <p class="m-0">
          Family copy:
          <span class="text-[var(--ui-fg)]">{{
            family.isSnapshotDirty.value ? "Needs save" : "Up to date"
          }}</span>
        </p>
      </div>
