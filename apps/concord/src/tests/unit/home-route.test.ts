...
describe("RouteHome landing page", () => {
  it("renders the Concord landing content and ledger artifact section", async () => {
    ...
    expect(text).toContain("Build apps on truth, not storage.");
    ...
    expect(text).toContain("A history artifact you can inspect and tamper");
    ...
    expect(text).toContain(
      "Concord will not derive runtime state from corrupted or unverified history. Integrity is not optional.",
    );
    expect(text).toContain("Concord sits at the top of a composable stack.");
    expect(text).toContain("Explore Seal");
    expect(text).toContain("Explore Armour");
    expect(text).toContain("Explore Ledger");
    expect(text).toContain("Build an app without giving storage authority");
    expect(text).toContain("For developers");
    expect(text).toContain('await app.command("todo.create-item"');
    expect(text).toContain("Create first todo");

    expect(text.indexOf("Concord sits at the top of a composable stack.")).toBeLessThan(
      text.indexOf("Build an app without giving storage authority"),
    );

    expect(text).toContain("JavaScript");
    expect(text).toContain("Ledger + Seal");
    expect(text).toContain("Replay plugins");
    ...
    await clickTab(wrapper, "Replay plugins");

    expect(wrapper.text()).toContain("plugins: [createTodoPlugin(), lokiPlugin]");
