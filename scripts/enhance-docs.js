...
function parseFunctionSignature(line) {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      // Parse parameters
      const parsedParams = params.split(',').map(p => {
        const param = p.trim();
...
function generateJSDocForFunction(funcInfo) {
  ...
  if (isAsync) {
    lines.push(` * @returns Promise that resolves to ${returnType.replace('Promise<', '').replace('>', '')}`);
  } else {
...
module.exports = { addEnhancedJSDocToFile, parseFunctionSignature, generateJSDocForFunction };
