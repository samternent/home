...
const sidebarClasses = computed(() => {
  ...
  const positionClasses = {
    left: ["left-0", props.modelValue ? "translate-x-0" : "-translate-x-full"],
    right: ["right-0", props.modelValue ? "translate-x-0" : "translate-x-full"],
  };

  // Variant classes
  const variantClasses = {
    default: ["bg-base-100", "border-r border-base-300", "shadow-lg"],
    glass: ["bg-base-100/90", "backdrop-blur-xl border-r border-base-100/20", "shadow-xl"],
    minimal: ["bg-base-100/95", "backdrop-blur-sm"],
    bordered: ["bg-base-100", "border-r-2 border-base-300", "shadow-lg"],
  };
...
<template>
  <!-- Overlay -->
  <div v-if="overlay" :class="overlayClasses" @click="handleOverlayClick" />
