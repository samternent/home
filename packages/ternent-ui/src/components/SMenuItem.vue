...
const itemClasses = computed(() => {
  ...
  if (props.active) {
    baseClasses.push("bg-primary-50", "text-primary-700", "font-medium");
  } else {
    baseClasses.push("text-base-content", "hover:bg-base-200", "hover:text-base-content");
  }
  ...
  <component :is="component" v-bind="componentProps" @click="handleClick">
    ...
    <div v-if="badge || $slots.action" class="flex-shrink-0 ml-3">
      <slot name="action">
        <span
          v-if="badge"
          class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-base-300 text-base-content"
        >
          {{ badge }}
        </span>
      </slot>
