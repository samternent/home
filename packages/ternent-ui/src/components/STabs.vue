...
const props = defineProps({
  ...
  size: {
    ...
    default: "micro",
    validator: (value) =>
      ["nano", "micro", "tiny", "small", "medium", "sm", "md", "lg"].includes(value),
  },
  sticky: {
    type: Boolean,
    default: false,
...
const typeClasses = computed(() => ({
  ...
  pills:
    "rounded-lg hover:bg-base-200/60 transition-all duration-200 ease-out border border-transparent hover:border-base-300/40",
  bordered:
    "border border-base-300/40 hover:border-base-300/60 hover:bg-base-50/50 transition-all duration-200 ease-out rounded-lg",
  segmented:
    "first:rounded-l-lg last:rounded-r-lg border-r border-base-300/30 last:border-r-0 hover:bg-base-100/80 transition-all duration-200 ease-out relative",
  stripe: "relative text-base-content/60 hover:text-base-content",
}));

const activeClasses = computed(() => ({
  underline:
    "text-primary font-medium after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-gradient-to-r after:from-primary after:to-secondary after:rounded-full",
  ...
  stripe:
    "text-base-content font-semibold relative after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-gradient-to-r after:from-primary after:to-secondary",
}));
</script>
<template>
  <div
  ...
  >
    <nav
      ...
      :class="{
        ...
        'bg-gradient-to-r from-base-200/60 to-base-300/40 rounded-2xl p-1.5 border border-base-300/30':
          type === 'segmented',
      }"
    >
      <RouterLink
        ...
        :class="[
          ...
          typeClasses[type],
          {
            [activeClasses[type]]: (!exact && isActiveLink(t)) || (exact && isExactActiveLink(t)),
          },
        ]"
      >
        <!-- Icon if provided -->
