...
function applyGroupUpsert(state: PermissionState, entry: Entry): PermissionState {
  const payload = parsePayload<GroupUpsertPayload>(groupUpsertPayloadSchema, entry.payload);
...
function applyGroupMember(
  ...
  const payload = parsePayload<GroupMemberPayload>(groupMemberPayloadSchema, entry.payload);

  const group = state.groups[payload.groupId];
  if (!group) {
    throw new PermissionRegistryError("INVALID_TARGET", `Missing group ${payload.groupId}`);
  }
  if (!isAuthorizedGroupChange(state, entry.author, payload.groupId)) {
    throw new PermissionRegistryError(
      "UNAUTHORIZED_GROUP_UPDATE",
...
function applyPermGrant(state: PermissionState, entry: Entry, order: number): PermissionState {
  const payload = parsePayload<PermGrantPayload>(permGrantPayloadSchema, entry.payload);
...
function applyPermRevoke(state: PermissionState, entry: Entry, order: number): PermissionState {
  const payload = parsePayload<PermRevokePayload>(permRevokePayloadSchema, entry.payload);
