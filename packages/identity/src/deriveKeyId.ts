...
export async function deriveKeyIdFromPublicKeyBase64(publicKeyBase64: string): Promise<string> {
  return hashData(normalizePublicKeyBody(publicKeyBase64));
}

export async function deriveKeyIdFromPublicKeyPem(publicKeyPem: string): Promise<string> {
  return hashData(stripIdentityKey(publicKeyPem));
}

export async function deriveKeyIdFromPublicKey(publicKey: CryptoKey): Promise<string> {
